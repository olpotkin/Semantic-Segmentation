...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
	...
	l7_up = tf.layers.conv2d_transpose(l7_1x1, num_classes, 4,
									   strides=(2, 2),
									   padding='same',
									   kernel_initializer=tf.random_normal_initializer(stddev=0.01),
									   kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
	...
	# Add skip connection
	l4_skip = tf.add(l7_up, l4_1x1)

	# Deconvolution (upsampling)
	l4_up = tf.layers.conv2d_transpose(l4_skip, num_classes, 4,
									   strides=(2, 2),
									   padding='same',
									   kernel_initializer=tf.random_normal_initializer(stddev=0.01),
									   kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
	...
	# Add skip connection
	l3_skip = tf.add(l4_up, l3_1x1)

	# Deconvolution (upsampling)
	nn_last_layer = tf.layers.conv2d_transpose(l3_skip, num_classes, 16,
											   strides=(8, 8),
											   padding='same',
											   kernel_initializer=tf.random_normal_initializer(stddev=0.01),
											   kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
	...
	for i in range(epochs):
		...
		for image, label in get_batches_fn(batch_size):
			...
			_, loss = sess.run([train_op, cross_entropy_loss],
							   feed_dict={input_image: image, correct_label: label, keep_prob: 0.6, learning_rate: 0.0005})
